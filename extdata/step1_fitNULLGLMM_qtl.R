...
option_list <- list(
  ...
  make_option("--useGPU",
    type = "logical", default = FALSE,
    help = "Optional. Whether to use GPU acceleration for GRM matrix-vector products. Requires CUDA toolkit and GPU-enabled package build. [default=FALSE]"
  )
)
...
# Add useGPU parameter if supported by the package
useGPU_supported <- "useGPU" %in% names(formals(fitNULLGLMM_multiV))
if (useGPU_supported) {
  args_list$useGPU <- opt$useGPU
  if (opt$useGPU) {
